    ...
    handle(message: ClientCheat, player: Player): boolean {
        ...
        if (!Environment.node.production && player.staffModLevel >= 4) {
            ...
            if (cmd[0] === Environment.node.debugProcChar) {
                ...
                const debugProc = cmd.slice(1);
                const script = ScriptProvider.getByName(`[debugproc,${debugProc}]`);
                if (!script) {
                    return false;
                }
                    ...
                    try {
                        switch (type) {
                            case ScriptVarType.STRING: {
                                const value = debugProc === 'item' && script.info.parameterTypes.length === 1 ? args.join(' ') : args.shift();
                                params[i] = value ?? '';
                                break;
                            }
